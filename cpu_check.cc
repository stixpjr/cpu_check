...
inline void __movsb(char *dst, const char *src, size_t size) {
	__asm__ __volatile__("rep movsb" : "+D"(dst), "+S"(src), "+c"(size)
                        : : "memory");
}
#endif
...
class Worker {
  public:
	Worker(std::vector<std::string> *words, int tid) : tid_(tid), words_(words), rndeng_(std::random_device()()) {
	}
	~Worker() {}
	void Run();
...
void Worker::Run() {
	...
	std::vector<compressorItem> compressors = {
		{
			...
			[](std::string *o, const std::string& s) {
				...
				o->resize(olen);
				int err = compress2((Bytef*)&(*o)[0], &olen,
									(Bytef*)s.data(), s.size(), Z_BEST_SPEED);
				if (err != Z_OK) {
					LOG(DEBUG) << "zlib compression failed: " << err
								<< " srclen: " << s.size() << " destlen: " << o->size();
					return err;
				}
				o->resize(olen);
				return 0;
			...
			[](std::string *o, const std::string& s) {
				uLongf olen = o->size();
				int err = uncompress((Bytef*)&(*o)[0], &olen, (Bytef*)s.data(), s.size());
				if (err != Z_OK) {
	...
	std::normal_distribution<double> buf_size_dist((BUF_MAX - BUF_MIN) / 2, (BUF_MAX - BUF_MIN) / 3);
	...
	while (!exiting) {
		...
		for (uint32_t i = 0; i < hashers.size(); i++) {
			...
			// LOG(DEBUG) << "Hash " << hashers[i].name << ": " << hashes[i] << "."; XXX
		}
		...
		EVP_CipherInit_ex(cipher_ctx, EVP_aes_256_gcm(), NULL, key, (unsigned char*) ivec.data(), 1);
		
		dst_buf.resize(src_buf.size());
		if (EVP_CipherUpdate(cipher_ctx, (unsigned char *)&dst_buf[0], &enc_len, (unsigned char *)src_buf.data(), src_buf.size()) != 1) {
		...
		dst_buf.resize(src_buf.size());
		int offset = std::uniform_int_distribution<int>(1, src_buf.size() - 1)(rndeng_);
#if defined(__i386__) || defined(__x86_64__)
		...
		if (std::thread::hardware_concurrency() > 1) {
			...
			int cpuoff = std::uniform_int_distribution<int>(0, cpus.size()-1)(rndeng_);
			newcpu = cpus[cpuoff];
		...
		int dec_len;
		EVP_CipherInit_ex(cipher_ctx, EVP_aes_256_gcm(), NULL, key, (unsigned char*) ivec.data(), 0);
		dst_buf.resize(src_buf.size());
		...
		if (EVP_CipherUpdate(cipher_ctx, (unsigned char *)&dst_buf[0], &dec_len,
				(unsigned char *)src_buf.data(), src_buf.size()) != 1) {
		...
		if (err) {
			LOG(ERROR) << "tid " << tid_ << " cpu " << newcpu << " " << gen.name
						<< " size " << bufsize
						<< " " << comp.name << " uncompression failed: " << err;
			errorCount++;
			continue;
		}
		if (src_buf.size() != bufsize) {
			LOG(ERROR) << "tid " << tid_ << " cpu " << newcpu << " " << gen.name
						<< " " << comp.name
						<< " uncompressed buffer size mismatch: should be "
						<< bufsize << " got " << src_buf.size() << ".";
			errorCount++;
			continue;
		}
		...
		for (uint32_t i = 0; i < hashers.size(); i++) {
			...
			if (hashes[i] != hash) {
				LOG(ERROR) << "tid " << tid_ << " cpu " << newcpu << " "
							<< gen.name << " size " << bufsize << " " << comp.name << " "
...
int main(int argc, char **argv) {
	...
	LOG(ERROR) << errorCount.load() << " ERRORS, "
		<< successCount.load() << " SUCCESSES.";
	LOG(INFO) << "Exiting.";
	exit(errorCount != 0);
}
